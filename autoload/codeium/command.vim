...
function! s:commands.Auth(...) abort
  ...
  while empty(api_key) && tries < 3
    ...
    " For Windows systems behind a corporate proxy there may be trouble verifying the 
    " SSL certificates. The easiest thing is just to ignore the SSL revocation check.
